...
def agregar_plato():
    ...
    # Borrar un plato por su ID
    @app.route('/plato/<int:plato_id>', methods=['DELETE'])
    def borrar_plato(plato_id):
        plato = MenuItem.query.get(plato_id)
        if not plato:
    ...
    def borrar_categoria(categoria_id):
        categoria = Menu.query.get(categoria_id)
        if not categoria:
            return jsonify({"error": "Categoría no encontrada"}), 404
...
def obtener_detalles_pedido(pedido_id):
    detalles = OrderDetail.query.filter_by(orderID=pedido_id).all()
    serialized_detalles = [detalle.serialize() for detalle in detalles]
    return jsonify({"detalles": serialized_detalles})
